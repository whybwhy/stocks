 ...
 * 시간대 외 접근 시 403. {@code /stocker} 등은 {@link ChartboyPublicAccess} 와 별도로 제외.
 */
@Component
public class PublicPriceAlertTimeWindowFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (appProperties.isWithinPriceAlertPublicTimeWindow(appProperties.nowInPriceAlertPublicWindowZone())) {
            filterChain.doFilter(request, response);
            return;
        }
