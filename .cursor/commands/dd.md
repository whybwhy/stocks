...
- **`label` 종목 관련만 적재·갱신**: `price_alerts` 와 동일 — **해당 종목의 타점·가격·차트 맥락이 아닌 라벨은 행을 만들지 않고**, `ON CONFLICT … DO UPDATE` 대상에도 넣지 않는다. `target_price` 가 `NULL` 이라도 **종목 타점 설명이 없으면** 넣지 않는다.
