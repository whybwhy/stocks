...
- **`label` 원문 보존**: 사용자가 적어 준 줄(이모지·기호·줄넘김 포함)을 **요약·축약·재구성 없이 그대로** 넣는다. (SQL 따옴표만 `''`로 이스케이프) 한 줄에 여러 가격이 있으면 행마다 동일한 원문 줄을 라벨에 넣어도 된다.
...
- **`label` 원문 보존**: `price_alerts` 와 동일하게 사용자가 보낸 줄을 **요약·축약·재구성 없이 그대로** 넣는다. 사용자 메시지의 줄넘김(`\n`)·이모지·기호도 그대로. `ON CONFLICT … DO UPDATE` 시 `label = EXCLUDED.label` 로 최신 원문이 반영되도록 한다.
